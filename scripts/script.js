...
document.querySelector("main").addEventListener("scroll", (event) => {
	if (isInViewport(treasureRoom)){
		...
		document.querySelector(".items > img").src = "images/items/" + imageSrc;
		document.querySelector(".items > img").style.display = "block";
	}
});
	...
	if (isInViewport(spikeRoom)){
		...
		if (health < 1){
			...
			setTimeout(() => {
				...
				document.querySelector(".items > img").style.display = "none";
			}, 1500);
		}
	}
});
